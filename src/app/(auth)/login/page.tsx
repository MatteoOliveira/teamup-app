...
export default function LoginPage() {
  ...
  async function handleLogin(e: React.FormEvent) {
    ...
    if (error) {
      if (error.message.includes("Email not confirmed")) {
        setError("Email non confirmé. Confirme ton compte dans le dashboard Supabase.");
  ...
  return (
    <div
      style={{
        minHeight: "100vh",
        background: "linear-gradient(160deg, #1A2B4A 0%, #243757 55%, #FF6B35 150%)",
    ...
    >
      {/* Decorative blobs */}
      <div style={{ position: "fixed", top: -80, right: -80, width: 280, height: 280, borderRadius: "50%", background: "rgba(255,107,53,0.12)", pointerEvents: "none" }} />
      <div style={{ position: "fixed", bottom: -60, left: -60, width: 200, height: 200, borderRadius: "50%", background: "rgba(46,196,182,0.1)", pointerEvents: "none" }} />
      ...
      <div className="flex flex-col items-center" style={{ marginBottom: 36 }}>
        ...
        <p style={{ fontSize: 14, fontWeight: 600, color: "rgba(255,255,255,0.55)", marginTop: 6 }}>Connecte-toi pour jouer</p>
      </div>
      ...
      >
        ...
        <form onSubmit={handleLogin}>
          ...
          <div style={{ marginBottom: 14 }}>
            <label style={{ fontSize: 13, fontWeight: 700, color: "#1A2B4A", display: "block", marginBottom: 6 }}>
              Email
            </label>
            <div className="flex items-center gap-2"
              style={{ height: 48, borderRadius: 12, border: "1.5px solid #E5E8EE", padding: "0 14px", background: "#fff" }}>
          ...
          <button
            ...
            style={{
              ...
              background: email && password ? "linear-gradient(135deg, #FF6B35, #E5551F)" : "#F1F3F7",
              color: email && password ? "#fff" : "#8A93A6",
              border: "none", cursor: email && password ? "pointer" : "not-allowed",
              fontSize: 15, fontWeight: 800, letterSpacing: -0.2,
              boxShadow: email && password ? "0 6px 18px rgba(255,107,53,0.35)" : "none",
        ...
        <p style={{ textAlign: "center", fontSize: 13, fontWeight: 600, color: "#8A93A6", marginTop: 18 }}>
          ...
          <Link href="/register" style={{ color: "#FF6B35", fontWeight: 700, textDecoration: "none" }}>
            Créer un compte
          </Link>
        </p>
      </div>
